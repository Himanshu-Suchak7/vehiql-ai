					...
					{routes.map((route) => {
						return (
							<Link
								...
								href={route.href}
								className={cn(
									"flex flex-col items-center justify-center gap-1 text-[10px] font-medium transition-all px-1",
									pathname === route.href 
										? "text-blue-700" 
							...
							>
								<route.icon className="h-5 w-5" />
								<span className="truncate max-w-[80px] text-center">{route.label}</span>
							</Link>
						);
					})}
				</div>
